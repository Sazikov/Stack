...
int StackCtor (struct stack* stk, const int capacity)
{
    ...
    Poison (stk, 1, stk->capacity + canary);
...
int StackDecrease (struct stack* stk)
{
    if (stk == NULL)
    {
        stk->errors |= STACKPTRERR;
...
int StackPush (struct stack* stk, const elem_t elem)
{
    ...
    if (stk->size == stk->capacity)
    {
        stk->capacity *= 2;
        StackIncrease (stk);
        Poison (stk, stk->size + 1, stk->capacity + 1);
    }
 ...
 int StackIncrease (struct stack* stk)
{
    if (stk == NULL)
    {
        stk->errors |= STACKPTRERR;
...
int StackVerificator (struct stack* stk)
{
    ...
    if (stk->data == NULL)
        stk->errors |= DATAERR;
    if (stk->size > stk->capacity)
        stk->errors |= SIZEBIGGERTHANCAPERR;
...
{
    return stk->left_canary * 1 + stk->size * 2 + stk->capacity * 2 + stk->errors * 4 + stk->Data_Hash * 5 + stk->right_canary * 6 + (long int) *stk->data * 7;
}
#endif
